...
def get_MFCC(sr, audio):
    meramfcc = []
    meramfcc = preprocessing.scale(mfcc(audio, sr, 0.025, 0.01, 100, appendEnergy=False))
    return(meramfcc)

def getRandomList(len_feature):
    i = 0
    random_list = []
    while i < 100:
...
def train_SVM(data_path, dtype):
    ...
    for file in file_list_male:
        ...
        random_list = getRandomList(len(feature)-1)
    ...
    joblib.dump(clf, 'test_round_2.pkl')


def test_SVM(data_path, dtype):
    ...
    clf = joblib.load('test_round_2.pkl')
    ...
    for file in file_list_male:
        ...
        scores = clf.predict(test_feature)
        print(file)
...
if __name__ == '__main__':
    train_SVM("./pre_data", np.int16)
    test_SVM('./data', np.int16)
